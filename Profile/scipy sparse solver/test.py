...
def main():
    ...
    p_star = zeros((imax, jmax))        # intermediate pressure
    p_prime = zeros((imax, jmax))       # pressure correction
    rhsp = zeros((imax, jmax))          # right hand side vector of pressure correction equation
    div = zeros((imax, jmax))

    # Vertical velocity
    v_star = zeros((imax, jmax + 1))    # intermediate velocity
    v = zeros((imax, jmax + 1))         # final velocity
    d_v = zeros((imax, jmax + 1))       # velocity correction coefficient

    # Horizontal Velocity
    u_star = zeros((imax + 1, jmax))    # intermediate velocity
    u = zeros((imax + 1, jmax))         # final velocity
